...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while temp_place != hive:
            ...
            temp_place, place_count = temp_place.entrance, place_count + 1
        return None
...
# BEGIN Problem 7
class WallAnt(Ant):
    """WallAnt, which is an ant that does nothing but has a large armor value."""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            while temp_place is not None:
                if temp_place.ant:
                    if temp_place.ant not in self.doubled_ant:
                        temp_place.ant.damage *= 2
                        self.doubled_ant.append(temp_place.ant)
                    if temp_place.ant.container and temp_place.ant.ant:
                        if temp_place.ant.ant not in self.doubled_ant:
                            temp_place.ant.ant.damage *= 2
                            self.doubled_ant.append(temp_place.ant.ant)
...
def apply_effect(effect, bee, duration):
    ...
    action = bee.action
    def effected_bee(colony):
        nonlocal duration
        if duration > 0:
            duration -= 1
            return  effect(action)(colony)
        return action(colony)
    bee.action = effected_bee
